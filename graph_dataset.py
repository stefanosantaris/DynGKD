...
class GraphDataset(Dataset):
    def __init__(self, graphs, features, adjs, num_time_steps, context_pairs, negative_sample = 1, window=0):
        self.graphs = graphs
        ...
        self.window = window
        self.min_t = 0
        self.train_nodes = list(self.graphs[num_time_steps - 1].nodes())
    ...
    def construct_degs(self):
        ...
        for i in range(0, self.num_time_steps):
            ...
            degs.append(deg)
        if self.window > 0:
            self.min_t =max(self.num_time_steps - self.window - 1, 0)
        return degs[self.min_t:]


    def __len__(self):
        return self.graphs[self.num_time_steps - 1].number_of_nodes() - 1
    ...
    def __getitem__(self,idx):
        ...
        for t in range(self.min_t, self.num_time_steps):
            node_1 = []
            node_2 = []
